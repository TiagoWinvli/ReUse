...
function setupSwipeEvents(card) {
    ...
    // Listeners de movimento e soltar são globais (no 'document')
...
function startDrag(e) {
    ...
    // Pega a posição inicial correta (mouse ou toque)
...
function drag(e) {
    ...
    e.preventDefault(); // Essencial para o celular não rolar a tela

    // Verifica se é um evento de mouse ou de toque e pega as coordenadas
    if (e.type.includes('mouse')) {
        currentX = e.clientX;
        currentY = e.clientY;
    } else {
        // Para 'touch', precisamos checar se 'e.touches' existe e tem pelo menos um toque
        if (e.touches && e.touches.length > 0) {
            currentX = e.touches[0].clientX;
            currentY = e.touches[0].clientY;
        } else {
            // Se não houver 'touches', pode ser um evento 'touchmove' fantasma, então paramos.
            return;
        }
    }
